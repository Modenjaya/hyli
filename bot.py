...
class PharosTestnet:
    def __init__(self):
        ...
        self.wrap_amount = 0
        # === START MODIFICATION ===
        # Tambahkan variabel untuk menyimpan persentase swap
        self.swap_percentage = 0.0
        # === END MODIFICATION ===
...
"""
    ...
    # === START MODIFICATION ===
    # Ubah perform_swap agar menghitung amount_in berdasarkan persentase saldo
    async def perform_swap(self, account, address, token_in_symbol, token_out_symbol): # Hapus parameter amount_in
        try:
            web3 = await self.get_web3()
            if not web3:
                return None, None
            ...
            current_balance = await self.get_token_balance(address, token_in_symbol)
            if current_balance is None or current_balance <= 0:
                self.log(f"No sufficient {token_in_symbol} balance found for swap.", indent=2, color=Fore.RED)
                return None, None
            
            # Hitung amount_in berdasarkan persentase
            amount_in_float = current_balance * (self.swap_percentage / 100)
            
            if amount_in_float <= 0:
                self.log(f"Calculated swap amount is zero or negative ({amount_in_float:.6f} {token_in_symbol}). Skipping swap.", indent=2, color=Fore.RED)
                return None, None

            self.log(f"Attempting to swap {amount_in_float:.6f} {token_in_symbol} ({(self.swap_percentage):.2f}% of balance) to {token_out_symbol}", indent=2, color=Fore.CYAN)

            if token_in_symbol != "PHRS":
                await self.approving_token(account, address, self.SWAP_ROUTER_ADDRESS, token_in, amount_in_float) # Gunakan amount_in_float
            ...
            amount_in_wei = int(amount_in_float * (10 ** decimals)) # Konversi ke wei
            ...
            # Perhatikan penanganan value untuk PHRS native token
            value_to_send = amount_in_wei if token_in_symbol == "PHRS" else 0
            estimated_gas = swap_data.estimate_gas({"from": address, "value": value_to_send})
            max_priority_fee = web3.to_wei(1, "gwei")
    ...
    # === START MODIFICATION ===
    # Ubah process_perform_swap agar tidak lagi menerima amount_in, karena akan dihitung di perform_swap
    async def process_perform_swap(self, account, address, token_in_symbol, token_out_symbol, iteration, total_iterations):
        self.log(f"Swap {iteration}/{total_iterations}: {token_in_symbol}/{token_out_symbol}", indent=2, color=Fore.YELLOW)
        # Saldo akan diambil di dalam perform_swap
        # amount_in tidak lagi dibutuhkan di sini sebagai parameter
        tx_hash, block_number = await self.perform_swap(account, address, token_in_symbol, token_out_symbol)
        if tx_hash and block_number:
            self.log(f"Swap from {token_in_symbol} to {token_out_symbol} Success", indent=2, color=Fore.GREEN)
            self.log(f"Block: {block_number}", indent=2, color=Fore.CYAN)
            self.log(f"Tx: {tx_hash}", indent=2, color=Fore.CYAN)
            self.log(f"Explorer: {self.EXPLORER_URL}/{tx_hash}", indent=2, color=Fore.CYAN)
            return True
    ...
    # === START MODIFICATION ===
    # Ubah generate_swap_option agar tidak lagi mengembalikan amount_in hardcoded
    def generate_swap_option(self):
        ...
        token_in_symbol, token_out_symbol = random.choice(swap_pairs)
        return token_in_symbol, token_out_symbol
    # === END MODIFICATION ===
        ...
        for i in range(self.auto_all_count):
            ...
            if wrap_success:
                ...
                if unwrap_success:
                    ...
                    # Swap
                    # === START MODIFICATION ===
                    token_in_symbol, token_out_symbol = self.generate_swap_option()
                    await self.process_perform_swap(account, address, token_in_symbol, token_out_symbol, i+1, self.auto_all_count)
                    # === END MODIFICATION ===
                    if True: # Asumsi swap berhasil atau kita tidak peduli hasilnya untuk lanjut ke LP
                    ...
                    # else:
                        # self.log("Skipping liquidity due to swap failure", indent=2, color=Fore.RED) # Ini akan dicetak di dalam process_perform_swap
                # else:
                    # self.log("Skipping swap and liquidity due to unwrap failure", indent=2, color=Fore.RED) # Ini akan dicetak di dalam process_perform_unwrapped
            # else:
                # self.log("Skipping unwrap, swap, and liquidity due to wrap failure", indent=2, color=Fore.RED) # Ini akan dicetak di dalam process_perform_wrapped
    ...
    async def process_option_4(self, account, address):
        ...
        for i in range(self.swap_count):
            # === START MODIFICATION ===
            token_in_symbol, token_out_symbol = self.generate_swap_option()
            await self.process_perform_swap(account, address, token_in_symbol, token_out_symbol, i+1, self.swap_count)
            # === END MODIFICATION ===
    ...
    def print_question(self):
        ...
        if option in [1, 2, 3]:
            ...
            if option == 3:
                ...
                # === START MODIFICATION ===
                # Prompt for swap percentage for "Auto All" option
                while True:
                    try:
                        swap_percentage = float(input(f"{Style.BRIGHT + Fore.CYAN}Enter percentage of balance to swap for Auto All (e.g., 0.1, 50, 100): ").strip())
                        if 0 < swap_percentage <= 100:
                            self.swap_percentage = swap_percentage
                            break
                        print(f"{Style.BRIGHT + Fore.RED}Percentage must be between 0 and 100.")
                    except ValueError:
                        print(f"{Style.BRIGHT + Fore.RED}Invalid input. Enter a float or decimal number.")
                # === END MODIFICATION ===
        elif option == 4:
            while True:
                try:
                    ...
                    print(f"{Style.BRIGHT + Fore.RED}Invalid input. Enter a number.")
            # === START MODIFICATION ===
            # Prompt for swap percentage for "Swap Tokens" option
            while True:
                try:
                    swap_percentage = float(input(f"{Style.BRIGHT + Fore.CYAN}Enter percentage of balance to swap (e.g., 0.1, 50, 100): ").strip())
                    if 0 < swap_percentage <= 100:
                        self.swap_percentage = swap_percentage
                        break
                    print(f"{Style.BRIGHT + Fore.RED}Percentage must be between 0 and 100.")
